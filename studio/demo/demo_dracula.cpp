...
int main() {
  ...
  // ════════════════════════════════════════════════════════════
  w << "## Sections";
  w << "$$ Color Palette / Pink #FF79C6, Purple #BD93F9, Cyan #8BE9FD, Green "
       "#50FA7B";
  w << "$$ Design Goal / Vivid accent colors on a deep dark "
       "background(#282A36)";
  ...
  w << "### Imperative Table";
  {
    ...
    std::string r3[4] = {".border()", "hex/Srgb", "#6272A4",
                         "Border/accent color"};
  ...
  w << "### Dracula Gradient";
  {
    ...
    std::cout << "  "
              << g.apply(
                     "The quick brown fox jumps over the lazy dog -- Dracula")
              << "\n";
  ...
  w << "### Palette Swatches";
  {
    ...
    Swatch sw[] = {{"Pink", "#FF79C6"},   {"Purple", "#BD93F9"},
  ...
  w.info("All TermWriter methods return *this for fluent chaining")
      ...
      .bullet(
          "Formatting: .bold() .italic() .dim() .strike() .underline() .code()")
