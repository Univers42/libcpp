...
int main() {
  ...
  // ════════════════════════════════════════════════════════════
  w << "## Sections";
  w << "$$ Compilation / c++ -std=c++17 -Wall -Wextra -Werror -Iinclude "
       "-pthread";
  w << "$$ Linking / c++ main.cpp -L. -lcpp -o my_program";
  w << "$$ Testing / make compile_studio && "
       "./build/bin/studio/tests/test_runner";
  ...
  w << "### Nord Frost Gradient";
  {
    ...
    std::cout << "  "
              << g.apply("Frost: calm, icy, focused -- the Nord experience")
              << "\n";
  ...
  w << "### Palette Swatches";
  {
    ...
    Swatch sw[] = {{"Frost1", "#8FBCBB"}, {"Frost2", "#88C0D0"},
