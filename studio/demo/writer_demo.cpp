...
int main() {
  ...
  w << "## Sections";
  w << "$$ Architecture / Modules: async, data, math, mem, net, str, term, "
       "test, util.";
  ...
  w.h3("Gradients");
  {
    ...
    std::cout << "  "
              << g2.apply(
                     "libcpp \xe2\x80\x94 a modern C++ utility library for 42")
              << "\n";
  ...
  w.h3("Color Manipulation");
  {
    ...
    for (int i = 1; i <= 4; ++i) {
      ...
      std::cout << "  +" << static_cast<int>(r * 100)
                << "%    = " << li.to_ansi_fg() << li.to_hex()
                << TermUtils::reset() << "   -" << static_cast<int>(r * 100)
                << "%    = " << da.to_ansi_fg() << da.to_hex()
                << TermUtils::reset() << "\n";
    }
  }
  std::cout << "\n";
  ...
  w.h3("Named Palette");
  {
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      std::cout << c.to_ansi_fg() << "\xe2\x96\x88\xe2\x96\x88 " << names[i]
                << TermUtils::reset();
      if (i < 9) std::cout << "  ";
    }
    std::cout << "\n\n";
