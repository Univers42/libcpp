...
int main()
{
	...
	ss.table()
		...
		.border_color("#39CCCC")
		.header_fg("#7FDBFF").header_bg("#0D1B2A").header_font(FONT_BOLD)
		.cell_fg("#E8E8E8").cell_bg("#0D1B2A").cell_alt_bg("#0F2232")
		.zebra(true).cell_has_bg(true).header_has_bg(true)
		.title_fg("#7FDBFF").title_bg("#0D1B2A").title_has_bg(true)
		.footer_fg("#39CCCC");
	...
	/* ── Register CUSTOM callout types ────────────────────────────── */
	{
		...
		e.width  = 68; e.pad_l = 1; e.pad_r = 1; e.margin_l = 1;
	...
	w.callout("tip",
		"Tip",
		"Use define_callout() to register custom callout",
	...
	w.callout("ocean",
		"Ocean Tip",
		"This callout uses a dolphin emoji glyph",
		"with an ocean-blue color scheme.");
	w.nl();

	w.callout("fire",
		"Fire Warning",
		"This callout uses a fire emoji glyph",
		"with warm orange/amber colors.");
	w.nl();

	w.callout("star",
		"Starred Bookmark",
		"Custom callout with a star glyph",
	...
	/* Section */
	w.h2("Section Blocks");
	w.section("Architecture", "StyleSheet pattern: declare rules, apply to TermStyle");
	w.section("Performance", "Zero-cost: all config resolved at construction time");
	w.nl();
	...
	w.h2("How to Personalize Everything");
	{
		...
		t.row3("Heading 1",     "h1",      "fg, bg, border, font, align, glyph, sep, pad, spacing");
		...
		t.row3("Custom callout","define_callout()", "fg, bg, border, glyph, font, width");
