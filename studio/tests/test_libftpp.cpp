...
static void test_worker_pool(libcpp::test::TestSuite& s) {
  std::atomic<int> counter{0};
  {
    ...
    for (int i = 0; i < 10; ++i) pool.addJob([&]() { counter++; });
    // Give workers time
...
static void test_message_consumer(libcpp::test::TestSuite& s) {
  ...
  consumer.registerHandler(
      1, [&](libcpp::net::Message& m) { m.buffer() >> received; });
...
static void test_pool_object_move(libcpp::test::TestSuite& s) {
  ...
  ASSERT_EQ(s, *b, 99);
  ASSERT_EQ(s, pool.available(),
            static_cast<size_t>(1));  // one released via move-assign
}
...
static void test_statemachine_invalid(libcpp::test::TestSuite& s) {
  ...
  bool threw = false;
  try {
    sm.transitionTo(GameState::Pause);
  } catch (const std::runtime_error&) {
    threw = true;
  }
  ASSERT_TRUE(s, threw);
...
static void test_timer_timeout(libcpp::test::TestSuite& s) {
  ...
  timer.setTimeout([&]() { fired = true; }, std::chrono::milliseconds(10));
...
static void test_timer_interval(libcpp::test::TestSuite& s) {
  std::atomic<int> ticks{0};
  libcpp::util::Timer timer;
  timer.setInterval([&]() { ticks++; }, std::chrono::milliseconds(10));
