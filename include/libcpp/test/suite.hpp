...
public:
	...
	void assert_eq_double(double a, double b, double eps,
						  const char* file, int line);
...
# define ASSERT_EQ(suite, a, b) \
	(suite).assert_eq_int(static_cast<long>(a), static_cast<long>(b), \
						  __FILE__, __LINE__)
...
# define ASSERT_NEQ(suite, a, b) \
	(suite).assert_neq_int(static_cast<long>(a), static_cast<long>(b), \
						   __FILE__, __LINE__)

# define ASSERT_LT(suite, a, b) \
	(suite).assert_lt(static_cast<long>(a), static_cast<long>(b), \
					  __FILE__, __LINE__)
