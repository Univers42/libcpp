...
class Result {
 public:
  ...
  Result() : _is_ok(false) { new (&_storage.error) E(); }
  ...
  Result<U, E> map(U (*fn)(const T&)) const {
    if (_is_ok) return Result<U, E>::ok(fn(_storage.val));
  ...
  Result<T, F> map_err(F (*fn)(const E&)) const {
    if (_is_ok) return Result<T, F>::ok(_storage.val);
