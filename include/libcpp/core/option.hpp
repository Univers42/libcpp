...
class Option {
 public:
  ...
  Option(const Option& other) : _has_value(other._has_value) {
    if (_has_value) new (&_storage.val) T(other._storage.val);
  }
  ...
  T& unwrap_or(T& fallback) { return _has_value ? _storage.val : fallback; }
  ...
  Option<U> map(U (*fn)(const T&)) const {
    if (_has_value) return Option<U>::some(fn(_storage.val));
