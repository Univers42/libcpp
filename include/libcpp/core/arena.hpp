...
class Arena {
 public:
  ...
  explicit Arena(std::size_t cap) : _capacity(cap), _count(0) {
    _data = static_cast<T*>(std::malloc(sizeof(T) * cap));
    _alive = new bool[cap];
    std::memset(_alive, 0, sizeof(bool) * cap);
  }

  Arena(const Arena& other) : _capacity(other._capacity), _count(other._count) {
    ...
    for (std::size_t i = 0; i < _capacity; ++i) {
      if (_alive[i]) new (&_data[i]) T(other._data[i]);
    }
  }
  ...
  bool is_alive(Index idx) const { return idx < _capacity && _alive[idx]; }
  ...
  /* ── find: returns index of first element equal to val, or capacity if not
   * found ── */
  Index find(const T& val) const {
    for (std::size_t i = 0; i < _capacity; ++i)
      if (_alive[i] && _data[i] == val) return i;
    return _capacity;
  }
