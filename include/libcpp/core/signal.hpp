...
/*   Updated: 2026/03/21 21:25:26 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
class FnSlot : public ISlot<A> {
 public:
  ...
  FnSlot& operator=(const FnSlot& o) {
    _fn = o._fn;
    return *this;
  }
  ~FnSlot() {}

  void call(const A& arg) {
    if (_fn) _fn(arg);
  }
...
class Signal {
 ...
 private:
  ...
  void _clear() {
    typedef typename std::map<ConnectionId, ISlot<A>*>::iterator It;
    for (It it = _slots.begin(); it != _slots.end(); ++it) delete it->second;
    _slots.clear();
  }
};
...
class Signal0 {
 public:
  ...
  void emit() const {
    ...
    for (It it = _fns.begin(); it != _fns.end(); ++it) it->second();
  }
