...
class Pool {
 public:
  Pool() {
    ...
    for (int i = 0; i < Capacity - 1; ++i) _next[i] = i + 1;
    _next[Capacity - 1] = -1;
    _free_head = 0;
    _used = 0;
    for (int i = 0; i < Capacity; ++i) _alive[i] = false;
  }
  ...
  Pool& operator=(const Pool& o) {
    if (this != &o) {
      ...
      for (int i = 0; i < Capacity; ++i) {
        _next[i] = o._next[i];
        _alive[i] = o._alive[i];
        if (_alive[i]) new (_ptr(i)) T(*o._cptr(i));
      }
    }
    return *this;
  }
 ...
 private:
  ...
  const T* _cptr(int i) const {
    return reinterpret_cast<const T*>(_storage + i * sizeof(T));
  }
