...
/*   Updated: 2026/03/21 21:56:50 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ProgressBar::ProgressBar() : _total(100), _current(0), _start(std::clock()) {}
...
void ProgressBar::set(std::size_t n) { _current = (n > _total) ? _total : n; }
...
std::string ProgressBar::render() const {
  ...
  if (!_label.empty())
    out << _style.label_color.to_ansi_fg() << _label << " "
        << TermUtils::reset();

  int filled = 0;
  if (_total > 0) filled = (int)(_style.width * _current / _total);
  int empty_count = _style.width - filled;
  if (empty_count < 0) empty_count = 0;

  out << _style.left;
  out << _style.fill_color.to_ansi_fg();
  for (int i = 0; i < filled; ++i) out << _style.fill;
  out << TermUtils::reset();
  for (int i = 0; i < empty_count; ++i) out << _style.empty;
  out << _style.right;
  ...
  if (!_postfix.empty()) out << " " << _postfix;
...
void ProgressBar::println() const { std::cout << render() << std::endl; }

/* ══════════════════════════════════════════════════════════════════════════
 *  Spinner
 * ═════════════════════════════════════════════════════════════════════════ */

Spinner::Spinner() : _kind(DOTS), _frame(0), _done(false) {}
...
std::string Spinner::render() const {
  if (_done) return _final_msg.empty() ? _label + " done" : _final_msg;
  std::size_t total = 0;
...
const char* Spinner::_frames(Kind k, std::size_t frame, std::size_t& total) {
  ...
  static const char* arc[] = {"\xe2\x97\x9c", "\xe2\x97\x9d", "\xe2\x97\x9e",
                              "\xe2\x97\x9f"};
  static const char* arrow[] = {"\xe2\x86\x90", "\xe2\x86\x91", "\xe2\x86\x92",
                                "\xe2\x86\x93"};
