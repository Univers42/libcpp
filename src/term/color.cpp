...
Srgb& Srgb::operator=(const Srgb& other) {
  if (this != &other) _color = other._color;
  return *this;
}
...
Srgb Srgb::from_hex(const std::string& hex) {
  ...
  if (h.length() < 6) return Srgb(0, 0, 0);
...
Srgb Srgb::parse(const std::string& s) {
  if (s.empty()) return Srgb(0, 0, 0);
  if (s[0] == '#' || (s.length() == 6 && std::isxdigit(s[0])))
    return from_hex(s);
  if (s.substr(0, 4) == "rgb(") {
    std::size_t p1 = s.find(',');
    std::size_t p2 =
        (p1 != std::string::npos) ? s.find(',', p1 + 1) : std::string::npos;
    std::size_t pe = s.find(')');
    if (p1 != std::string::npos && p2 != std::string::npos &&
        pe != std::string::npos) {
      unsigned char r = (unsigned char)std::atoi(s.substr(4, p1 - 4).c_str());
      unsigned char g =
          (unsigned char)std::atoi(s.substr(p1 + 1, p2 - p1 - 1).c_str());
      unsigned char b =
          (unsigned char)std::atoi(s.substr(p2 + 1, pe - p2 - 1).c_str());
      return Srgb(r, g, b);
    }
  }
  return named(s);
...
Srgb Srgb::named(const std::string& name) {
  ...
  if (colors.empty()) _init_named_colors(colors);
...
Srgb Srgb::mix(const Srgb& a, const Srgb& b, double ratio) {
  ...
  unsigned char r =
      (unsigned char)(a.get_r() * (1.0 - ratio) + b.get_r() * ratio);
  unsigned char g =
      (unsigned char)(a.get_g() * (1.0 - ratio) + b.get_g() * ratio);
  unsigned char bl =
      (unsigned char)(a.get_b() * (1.0 - ratio) + b.get_b() * ratio);
  return Srgb(r, g, bl);
}
...
void Srgb::set_r(unsigned char r) {
  _color = (_color & 0x00FFFF) | ((unsigned int)r << 16);
}
void Srgb::set_g(unsigned char g) {
  _color = (_color & 0xFF00FF) | ((unsigned int)g << 8);
}
...
Srgb Srgb::lighten(double factor) const {
  ...
  return Srgb(
      ...
      (unsigned char)_clampi((int)(get_b() + (255 - get_b()) * factor), 0,
                             255));
}

Srgb Srgb::darken(double factor) const {
  factor = _clamp(factor, 0.0, 1.0);
  return Srgb((unsigned char)_clampi((int)(get_r() * (1.0 - factor)), 0, 255),
...
Srgb Srgb::saturate(double factor) const {
  ...
  return Srgb((unsigned char)_clampi(
                  (int)(gray + (get_r() - gray) * (1.0 + factor)), 0, 255),
              (unsigned char)_clampi(
                  (int)(gray + (get_g() - gray) * (1.0 + factor)), 0, 255),
              (unsigned char)_clampi(
                  (int)(gray + (get_b() - gray) * (1.0 + factor)), 0, 255));
}

Srgb Srgb::desaturate(double factor) const { return saturate(-factor); }

Srgb Srgb::complement() const {
  return Srgb((unsigned char)(255 - get_r()), (unsigned char)(255 - get_g()),
...
Srgb Srgb::blend(const Srgb& other, double t) const {
  ...
  unsigned char r =
      static_cast<unsigned char>(get_r() + (other.get_r() - get_r()) * t);
  unsigned char g =
      static_cast<unsigned char>(get_g() + (other.get_g() - get_g()) * t);
  unsigned char b =
      static_cast<unsigned char>(get_b() + (other.get_b() - get_b()) * t);
  return Srgb(r, g, b);
}
...
std::string Srgb::to_ansi_fg() const {
  std::stringstream ss;
  ss << "\033[38;2;" << (int)get_r() << ";" << (int)get_g() << ";"
     << (int)get_b() << "m";
  return ss.str();
}
...
std::string Srgb::to_ansi_bg() const {
  std::stringstream ss;
  ss << "\033[48;2;" << (int)get_r() << ";" << (int)get_g() << ";"
     << (int)get_b() << "m";
  return ss.str();
}
...
Gradient& Gradient::operator=(const Gradient& other) {
  if (this != &other) _stops = other._stops;
  return *this;
}
...
Srgb Gradient::at(double t) const {
  if (_stops.empty()) return Srgb(0, 0, 0);
  if (_stops.size() == 1 || t <= 0.0) return _stops[0];
  if (t >= 1.0) return _stops[_stops.size() - 1];
  double scaled = t * (_stops.size() - 1);
  std::size_t idx = (std::size_t)scaled;
  double frac = scaled - idx;
  if (idx + 1 >= _stops.size()) return _stops[_stops.size() - 1];
...
std::vector<Srgb> Gradient::generate(int steps) const {
  ...
  for (int i = 0; i < steps; ++i) result.push_back(at((double)i / (steps - 1)));
  return result;
}

std::string Gradient::apply(const std::string& text) const {
  if (text.empty() || _stops.empty()) return text;
