...
TreeNode::TreeNode()
    : label_color(204, 204, 204), detail_color(128, 128, 128) {}
...
int TreeNode::count() const {
  int n = 1;
  for (std::size_t i = 0; i < children.size(); ++i) n += children[i].count();
  return n;
}
...
TreeStyle::TreeStyle(const TreeStyle& o)
    : branch(o.branch),
      ...
      pipe(o.pipe),
      blank(o.blank),
      show_counts(o.show_counts),
      branch_color(o.branch_color),
...
void TreeRenderer::_render_node(std::ostringstream& out, const TreeNode& node,
                                ...
                                int depth) const {
  if (_style.max_depth >= 0 && depth > _style.max_depth) return;
