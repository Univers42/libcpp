...
TableStyle::TableStyle()
    : border(BORDER_LIGHT),
      ...
      title_has_bg(false),
      footer_fg(140, 140, 150),
      footer_font(FONT_DIM),
      pad(1),
      outer_margin(0),
      max_col_width(0),
...
TableStyle::TableStyle(const TableStyle& o)
    : border(o.border),
      ...
      header_fg(o.header_fg),
      header_bg(o.header_bg),
      header_font(o.header_font),
      header_separator(o.header_separator),
      show_header(o.show_header),
      header_has_bg(o.header_has_bg),
      cell_fg(o.cell_fg),
      cell_bg(o.cell_bg),
      cell_alt_bg(o.cell_alt_bg),
      zebra(o.zebra),
      cell_has_bg(o.cell_has_bg),
      title_fg(o.title_fg),
      title_bg(o.title_bg),
      title_font(o.title_font),
      title_has_bg(o.title_has_bg),
      footer_fg(o.footer_fg),
      footer_font(o.footer_font),
      pad(o.pad),
      outer_margin(o.outer_margin),
      max_col_width(o.max_col_width),
      show_row_numbers(o.show_row_numbers),
      row_separator(o.row_separator),
      compact(o.compact),
      focus_style(o.focus_style) {}
...
Table::Table()
    : _ncols(0), _nrows(0), _has_header(false), _has_custom_style(false) {
...
Table::Table(int ncols)
    : _ncols(ncols), _nrows(0), _has_header(false), _has_custom_style(false) {
  std::memset(_col_align, 0, sizeof(_col_align));
  std::memset(_col_widths, 0, sizeof(_col_widths));
  std::memset(_forced_widths, 0, sizeof(_forced_widths));
  std::memset(_highlight, 0, sizeof(_highlight));
...
Table::Table(const Table& o)
  ...
  for (int r = 0; r < MAX_ROWS; ++r) {
    ...
    for (int c = 0; c < MAX_COLS; ++c) _rows[r][c] = o._rows[r][c];
  }
}
...
void Table::set_style(const TableStyle& s) {
  _style = s;
  _has_custom_style = true;
}
...
int Table::_total_width() const {
  ...
  if (_style.border != BORDER_NONE) tw += _ncols + 1; /* vertical bars */
  return tw;
}
...
std::string Table::_render_hline(const BoxChars& bc, const char* left,
...
std::string Table::_truncate(const std::string& text, int max_w) const {
  if (max_w <= 0 || TermUtils::vis_len(text) <= max_w) return text;
...
std::string Table::_align_cell(const std::string& text, int width,
                               int align) const {
  ...
  if (align == ALIGN_RIGHT) return TermUtils::spaces(total_pad) + text;
  return text + TermUtils::spaces(total_pad);
}

std::string Table::_render_row(const BoxChars& bc, const std::string* cells,
                               bool is_header, int row_idx) const {
  std::ostringstream out;
  bool hl =
      (!is_header && row_idx >= 0 && row_idx < MAX_ROWS && _highlight[row_idx]);
...
std::string Table::_render_title_bar(const BoxChars& bc) const {
  ...
  out << _margin();
  out << TermUtils::apply_fg(_style.border_color) << bc.v;
  if (_style.title_has_bg) out << TermUtils::apply_bg(_style.title_bg);
