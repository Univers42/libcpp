...
void Client::connect(const std::string& host, uint16_t port) {
  if (_connected) throw std::runtime_error("Client: already connected");

  _sockFd = ::socket(AF_INET, SOCK_STREAM, 0);
  if (_sockFd < 0) throw std::runtime_error("Client: socket() failed");
...
void Client::send(const Message& msg) {
  if (!_connected) throw std::runtime_error("Client: not connected");
...
void Client::_receiveLoop() {
  while (_receiving && _connected) {
    ...
    if (!_recvExact(_sockFd, buf.data(), totalLen)) break;
...
void Client::_sendRaw(int fd, const std::vector<uint8_t>& data) {
  ...
  while (sent < data.size()) {
    ...
    if (n <= 0) throw std::runtime_error("Client: send data failed");
