...
double Rng::next_double() {
  return static_cast<double>(next()) /
         static_cast<double>(static_cast<unsigned long>(-1));
}
...
Generator& Generator::operator=(const Generator& o) {
  if (this != &o) _rng = o._rng;
  return *this;
}
Generator::~Generator() {}

int Generator::gen_int(int lo, int hi) { return _rng.next_int(lo, hi); }
double Generator::gen_double(double lo, double hi) {
  return lo + _rng.next_double() * (hi - lo);
}
bool Generator::gen_bool() { return _rng.next_bool(); }
char Generator::gen_char() { return _rng.next_char(32, 126); }
char Generator::gen_alpha() {
  return _rng.next_bool() ? _rng.next_char('a', 'z') : _rng.next_char('A', 'Z');
}
...
std::string Generator::gen_email() {
  ...
  static const char* domains[] = {"example.com", "test.org", "mail.net",
                                  "dummy.io"};
...
FuzzRunner::FuzzRunner()
    : _iterations(100),
      _seed(static_cast<unsigned long>(std::time(0))),
      _max_failures(0) {}
FuzzRunner::FuzzRunner(const FuzzRunner& o)
    : _iterations(o._iterations),
      _seed(o._seed),
      _max_failures(o._max_failures) {}
...
FuzzRunner& FuzzRunner::iterations(int n) {
  _iterations = n;
  return *this;
}
FuzzRunner& FuzzRunner::seed(unsigned long s) {
  _seed = s;
  return *this;
}
FuzzRunner& FuzzRunner::max_failures(int n) {
  _max_failures = n;
  return *this;
}
