...
{
	if (this != &o) { tag = o.tag; arg_int = o.arg_int; }
	return *this;
}
...
{
	if (_count < MAX_CALLS)
		_calls[_count++] = CallRecord("", 0);
}

void Spy::call(const std::string& tag)
{
	if (_count < MAX_CALLS)
		_calls[_count++] = CallRecord(tag, 0);
}

void Spy::call(const std::string& tag, long arg)
{
	if (_count < MAX_CALLS)
		_calls[_count++] = CallRecord(tag, arg);
}
...
{
	if (this != &o) { _value = o._value; _calls = o._calls; }
	return *this;
}
StubInt::~StubInt() {}

long StubInt::get()        { ++_calls; return _value; }
...
std::string StubStr::get()                      { ++_calls; return _value; }
