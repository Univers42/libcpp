...
ScopeTimer::~ScopeTimer() {
  ...
  oss << ms;
  std::cerr << dim.to_ansi_fg() << "[timer] " << _label << ": " << oss.str()
            << " ms" << TermUtils::reset() << std::endl;
}
...
StopWatch::StopWatch(const StopWatch& o)
  ...
  for (int i = 0; i < _lap_count; ++i) _laps[i] = o._laps[i];
}
...
double StopWatch::elapsed_ms() const {
  if (_running) {
    std::clock_t now = std::clock();
    return _accumulated +
           1000.0 * static_cast<double>(now - _start) / CLOCKS_PER_SEC;
  }
  return _accumulated;
}
...
double StopWatch::lap() {
  ...
  if (_lap_count < MAX_LAPS) _laps[_lap_count++] = dt;
  _lap_start = now;
  return dt;
}
