...
IVector2 IVector2::operator+(const IVector2& rhs) const { return {x + rhs.x, y + rhs.y}; }
IVector2 IVector2::operator-(const IVector2& rhs) const { return {x - rhs.x, y - rhs.y}; }
IVector2 IVector2::operator*(int s) const { return {x * s, y * s}; }
IVector2 IVector2::operator/(int s) const
{
    if (s == 0)
        throw std::runtime_error("IVector2: division by zero");
    return {x / s, y / s};
}

IVector2& IVector2::operator+=(const IVector2& rhs) { x += rhs.x; y += rhs.y; return *this; }
IVector2& IVector2::operator-=(const IVector2& rhs) { x -= rhs.x; y -= rhs.y; return *this; }
IVector2& IVector2::operator*=(int s) { x *= s; y *= s; return *this; }
...
bool IVector2::operator==(const IVector2& rhs) const { return x == rhs.x && y == rhs.y; }
...
double IVector2::length() const { return std::sqrt(static_cast<double>(lengthSquared())); }
