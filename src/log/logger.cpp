...
Srgb level_color(Level lv) {
  switch (lv) {
    case LTRACE:
      return Srgb(128, 128, 128);
    case LDEBUG:
      return Srgb(0, 200, 255);
    case LINFO:
      return Srgb(0, 255, 0);
    case LWARN:
      return Srgb(255, 255, 0);
    case LERROR:
      return Srgb(255, 80, 80);
    case LFATAL:
      return Srgb(255, 0, 0);
  }
...
ConsoleLogger::ConsoleLogger(const ConsoleLogger& o)
    : ILogger(), _min_level(o._min_level) {}
...
void FileLogger::log(Level level, const std::string& msg) {
  ...
  *_file << "[" << _timestamp() << "] [" << level_name(level) << "] " << msg
         << std::endl;
}
...
TimestampDecorator::TimestampDecorator(ILogger* inner)
    : _inner(inner), _owns(false) {}
...
LogColorDecorator::LogColorDecorator(ILogger* inner)
    : _inner(inner), _owns(false) {}
