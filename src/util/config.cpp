...
Config::Config(const Config& o) : _count(o._count) {
  for (int i = 0; i < _count; ++i) _entries[i] = o._entries[i];
}
...
bool Config::save(const std::string& path) const {
  ...
  for (int s = 0; s < sec_count; ++s) {
    if (!sections[s].empty()) ofs << "[" << sections[s] << "]\n";
    for (int i = 0; i < _count; ++i) {
...
int Config::_find(const std::string& section, const std::string& key) const {
  for (int i = 0; i < _count; ++i) {
    if (_entries[i].section == section && _entries[i].key == key) return i;
  }
  return -1;
}
...
int Config::get_int(const std::string& section, const std::string& key,
  ...
  if (end == _entries[idx].value.c_str()) return default_val;
...
const ConfigEntry& Config::entry(int i) const {
  if (i < 0 || i >= _count) return _entries[0];
  return _entries[i];
}

void Config::clear() {
  for (int i = 0; i < _count; ++i) _entries[i] = ConfigEntry();
  _count = 0;
}
...
bool Config::remove(const std::string& section, const std::string& key) {
  ...
  for (int i = idx; i < _count - 1; ++i) _entries[i] = _entries[i + 1];
