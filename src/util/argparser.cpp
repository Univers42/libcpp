...
ArgDef::ArgDef(const ArgDef& o)
    : short_flag(o.short_flag),
      long_flag(o.long_flag),
...
ArgParser::ArgParser(const ArgParser& o)
    : _program(o._program),
      ...
      _def_count(o._def_count),
      _value_count(o._value_count),
      _pos_count(o._pos_count),
      _error(o._error) {
...
int ArgParser::_find_def(const std::string& f) const {
  for (int i = 0; i < _def_count; ++i) {
    if (_defs[i].short_flag == f || _defs[i].long_flag == f) return i;
  }
  return -1;
}
...
bool ArgParser::parse(int argc, char** argv) {
  ...
  for (int i = 0; i < _def_count; ++i) {
    std::string key =
        _defs[i].long_flag.empty() ? _defs[i].short_flag : _defs[i].long_flag;
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (past_flags || arg.empty() || arg[0] != '-') {
      if (_pos_count < MAX_POS) _positional[_pos_count++] = arg;
      continue;
    }
...
bool ArgParser::has(const std::string& f) const {
  ...
  int di = _find_def(f);
  if (di >= 0) {
    std::string key = _defs[di].long_flag.empty() ? _defs[di].short_flag
                                                  : _defs[di].long_flag;
...
std::string ArgParser::help() const {
  ...
  oss << _program;
  if (!_description.empty()) oss << " - " << _description;
  oss << "\n\nUsage: " << _program << " [options] [args...]\n\nOptions:\n";

  for (int i = 0; i < _def_count; ++i) {
    oss << "  ";
    if (!_defs[i].short_flag.empty()) oss << _defs[i].short_flag;
    if (!_defs[i].short_flag.empty() && !_defs[i].long_flag.empty())
      oss << ", ";
    if (!_defs[i].long_flag.empty()) oss << _defs[i].long_flag;
    if (_defs[i].takes_value) oss << " <value>";
    oss << "\n      " << _defs[i].description;
