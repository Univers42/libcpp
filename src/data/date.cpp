...
Date::Date(const Date& o) : _year(o._year), _month(o._month), _day(o._day) {}
...
Date Date::parse(const std::string& str, const std::string& fmt) {
  if (fmt == "YYYY-MM-DD" || fmt == "ISO") return _parse_iso(str);
  if (fmt == "DD/MM/YYYY") return _parse_dmy(str);
  if (fmt == "MM/DD/YYYY") return _parse_mdy(str);
  if (fmt == "YYYY/MM/DD") return _parse_ymd(str, '/');
...
bool Date::is_valid() const {
  ...
  if (_day < 1 || _day > days_in_month(_year, _month)) return false;
...
int Date::day_of_year() const {
  int d = _day;
  for (int m = 1; m < _month; ++m) d += days_in_month(_year, m);
  return d;
}
...
Date Date::add_years(int n) const {
  int y = _year + n;
  int d = _day;
  if (_month == 2 && _day == 29 && !is_leap_year(y)) d = 28;
...
long Date::to_epoch() const {
  ...
  static const int cum[] = {0,   31,  61,  92,  122, 153,
...
std::string Date::to_string(const std::string& fmt) const {
  if (fmt == "YYYY-MM-DD" || fmt == "ISO") return to_iso();
  if (fmt == "DD/MM/YYYY") {
    std::ostringstream o;
    o << std::setfill('0') << std::setw(2) << _day << "/" << std::setw(2)
      << _month << "/" << std::setw(4) << _year;
    return o.str();
  }
  if (fmt == "MM/DD/YYYY") {
    std::ostringstream o;
    o << std::setfill('0') << std::setw(2) << _month << "/" << std::setw(2)
...
Date Date::_parse_iso(const std::string& s) {
  if (s.length() < 10) throw std::invalid_argument("Invalid ISO date");
...
DateRange::DateRange(const DateRange& o) : _start(o._start), _end(o._end) {}
...
Date last_of_month(int y, int m) {
  return Date(y, m, Date::days_in_month(y, m));
}
