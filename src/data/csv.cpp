...
/*   Updated: 2026/03/21 21:27:51 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void CsvRow::set(const std::string& col, const std::string& val)
{
	...
	if (it != _col_map.end())
	{
		if (it->second >= _values.size())
			_values.resize(it->second + 1);
...
CsvOptions::CsvOptions()
	: delimiter(','), quote('"'), escape('\\'), has_header(true),
	  trim_whitespace(true), skip_empty_lines(true),
	  strict_quotes(false), skip_lines(0) {}
...
{ if (this != &o) _opt = o._opt; return *this; }
CsvParser::~CsvParser() {}
...
CsvDocument::CsvDocument(const CsvDocument& o)
	: _rows(o._rows), _headers(o._headers),
	  _col_map(o._col_map), _error(o._error) {}
...
bool CsvDocument::load_stream(std::istream& in, const CsvOptions& opt)
{
	...
	if (opt.has_header)
	{
		...
		for (std::size_t i = 0; i < _headers.size(); ++i)
			_col_map[_headers[i]] = i;
	}

	while (std::getline(in, line))
	{
		if (opt.skip_empty_lines && line.empty())
			continue;
...
const std::vector<std::string>& CsvDocument::headers() const    { return _headers; }
									   ...
									   const CsvOptions& opt) const
{
	...
	{
		if (f[i] == opt.quote) { esc += opt.quote; esc += opt.quote; }
		else esc += f[i];
	}
	esc += opt.quote;
	return esc;
