...
/*   Updated: 2026/03/21 21:41:48 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
DbColumn::DbColumn(const std::string& name, ColType::Type type,
                   Align::Type align)
...
void DbTable::calculate_widths() {
  for (std::size_t c = 0; c < _columns.size(); ++c) {
    ...
    for (std::size_t r = 0; r < _rows.size(); ++r) {
      std::string val =
          _columns[c].format_value(_rows[r].get_value(_columns[c].name()));
...
void Database::update_cell(std::size_t row, const std::string& column,
  ...
  if (row < rows.size()) rows[row].set_value(column, value);
}
...
std::vector<DbRow> Database::where(const std::string& column,
  ...
  for (std::size_t i = 0; i < rows.size(); ++i)
    if (rows[i].get_value(column) == value) results.push_back(rows[i]);
  return results;
}
...
DbTable db_sort_by(const DbTable& table, const std::string& column,
  ...
  DbTable result;
  const std::vector<DbColumn>& cols = table.columns();
  for (std::size_t i = 0; i < cols.size(); ++i) result.add_column(cols[i]);
  std::vector<DbRow> rows = table.rows();
  std::sort(rows.begin(), rows.end(), DbRowComparator(column, ascending));
  for (std::size_t i = 0; i < rows.size(); ++i) result.add_row(rows[i]);
  return result;
}
